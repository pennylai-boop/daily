...
/**
 * 只打勾（沒有書寫格式）的定期事項，三欄並排。
 *
 * 這種事項就是一個名字加一個勾，各佔一整列只是把畫面拉長。有書寫格式的事項不放進來：
 * 它們打勾後要在下方展開輸入欄位，並排會找不到對應關係。
 *
 * 欄數用**容器查詢**而不是螢幕寬度決定：同一個元件會出現在紀錄頁的寬欄（約 768px）與日曆頁
 * 桌機版的窄側欄（約 285px），用 `sm:` 之類的螢幕寬度斷點一定會有一邊排壞。
 * 容器窄到 256px 以下才退回兩欄。
 */
